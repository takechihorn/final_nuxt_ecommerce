      ...
      <div class="row">
        ...
        <div class="col-md-7 col-sm-7 col-xs-12">
          <div class="pl-6">
            ...
            >
            <v-btn class="ml-4" outlined tile>REMOVE FROM CART</v-btn>>
          </div>
        </div>
      </div>
      <div class="row">
                ...
                <v-hover v-slot:default="{ hover }" open-delay="200">
                  <v-card :elevation="hover ? 16 : 2">
                    <v-img
                      class="white--text align-end"
                      height="200px"
                      :src="require('../assets/img/shop/slider4.jpg')"
                    >
                ...
                <v-hover v-slot:default="{ hover }" open-delay="200">
                  <v-card :elevation="hover ? 16 : 2">
                    <v-img
                      class="white--text align-end"
                      height="200px"
                      :src="require('../assets/img/shop/slider3.jpg')"
                    >
...
export default {
  ...
  mounted() {
    console.log(this.$store.getters.cart.cartItems.length)
  },
  methods: {
    async getProductDetails() {
      try {
        await this.$store.dispatch('product/getProduct', this.$route.params.id)
        this.productName = this.data.getProduct.name
      } catch (error) {
        console.log({ error })
      }
    },
    async addToCart() {
      try {
        await this.$store.dispatch('cart/addToCart', this.$route.params.id)
      } catch (error) {
        console.log({ error })
      }
    },
